function [h_true,h_observed]=rir(fs, mic, n, r, rm, src,sigma2);
...
h_true=full(sparse(time(:),1,e(:)));       % Equivalent to equation 11

% Sum all time(:) that are the same reflection
ts = unique(time(:));
e_flat = e(:);
...
for t_i = 1:length(ts)
    ...
    es(t_i) = sum(e_flat(time(:) == t_local));
end

h_observed = full(sparse(round(ts+sqrt(sigma2)*randn(size(ts))),1,es));

% h_observed = full(sparse(round(time(:)+sqrt(sigma2)*randn(size(time(:)))),1,e(:)));
